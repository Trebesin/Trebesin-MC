...
export function main() {
    ...
    Commands.registerCommand('instakill', {
        ...
        run(sender) {
            ...
            if (instaKillStatus)
                serverPlayerData.instaKill[sender.id] = false;
            else
                serverPlayerData.instaKill[sender.id] = true;
    ...
    Commands.registerCommand('tphere', {
        ...
        run: (sender, parameter) => {
            for (let i = 0; i < parameter.players.length; i++) {
                parameter.players[i].teleport(sender.location, { dimension: sender.dimension });
            }
        }
    });
