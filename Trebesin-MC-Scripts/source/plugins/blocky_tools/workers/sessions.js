...
var _LeftClickDetect_instances, _LeftClickDetect_spawnEntity, _LeftClickDetect_playerData, _Session_player, _Session_clipboard, _Session_actionConfirmation;
//APIs:
...
class LeftClickDetect {
    constructor() {
        _LeftClickDetect_instances.add(this);
        _LeftClickDetect_playerData.set(this, {});
    }
    /**
    ...
    run(player) {
        /** @type {Mc.Entity} */
        let playerEntity = __classPrivateFieldGet(this, _LeftClickDetect_playerData, "f")[player.id];
        if (playerEntity == null) {
            __classPrivateFieldGet(this, _LeftClickDetect_instances, "m", _LeftClickDetect_spawnEntity).call(this, player, player.location);
        }
        else {
            try {
    ...
    stop(playerId) {
        let playerEntity = __classPrivateFieldGet(this, _LeftClickDetect_playerData, "f")[playerId];
        if (playerEntity !== null) {
            playerEntity.triggerEvent('trebesin:make_despawn');
            __classPrivateFieldGet(this, _LeftClickDetect_playerData, "f")[playerId] = null;
        }
    }
}
_LeftClickDetect_playerData = new WeakMap(), _LeftClickDetect_instances = new WeakSet(), _LeftClickDetect_spawnEntity = function _LeftClickDetect_spawnEntity(player, location) {
    ...
    //scale.value = 1.0;
    __classPrivateFieldGet(this, _LeftClickDetect_playerData, "f")[player.id] = playerEntity;
};
    ...
    Mc.system.runInterval(() => {
        for (const playerId in SessionStore) {
            ...
            if (getEquipedItem(player)?.typeId === 'trebesin:bt_blocky_axe') {
                ...
                player.onScreenDisplay.setActionBar(`[§aBlocky §2Tools§r] ${session.pointerBlockLocation != null ? `[${session.pointerBlockLocation.x},${session.pointerBlockLocation.y},${session.pointerBlockLocation.z}]` : ''}\n` +
...
class Session {
    ...
    constructor(player) {
        ...
        this.actionState = {};
        //### Other:
        _Session_clipboard.set(this, void 0);
        _Session_actionConfirmation.set(this, {});
        this.id = player.id;
        __classPrivateFieldSet(this, _Session_player, player, "f");
        __classPrivateFieldSet(this, _Session_clipboard, new ClipboardInstance(), "f");
        __classPrivateFieldGet(this, _Session_actionConfirmation, "f").pending = ActionPendingState.NONE;
        __classPrivateFieldGet(this, _Session_actionConfirmation, "f").confirm = null;
    ...
    async copySelection(permutations, exclusion) {
        ...
        await selection.getAllBlocks((blockLocation) => {
            try {
                ...
                const blockMatch = permutations.find(({ exactMatch, permutation }) => {
                    return ((exactMatch && block.permutation === permutation) || (!exactMatch && block.typeId === permutation.type.id));
                }) != null;
                if ((exclusion && blockMatch) || (!exclusion && !blockMatch))
                    return;
    ...
    pasteSelection(baseLocation, dimension, clipboardIndex) {
        ...
        clipboard.getAllBlocksProccessed((clipboardLocation, blockState) => {
            ...
            editBlock(block, blockState, { actorId: player.id, updateType: BlockHistoryUpdateTypes.blockyTools });
        }, clipboardIndex);
    }
    /**
    ...
    fillSelectionCorners(fillPermutation) {
        ...
        for (const blockLocation of selection.getAllCorners()) {
            ...
            setBlockPermutation(player.dimension.getBlock(blockLocation), fillPermutation, { actorId: player.id, updateType: 'blockyTools: player' });
        }
    }
    //## Action Confirm
    /**
    ...
    requestActionConfirmation() {
        if (__classPrivateFieldGet(this, _Session_actionConfirmation, "f").pending > ActionPendingState.NONE)
            throw new Error('There is an already pending confirmation for the session!');
        else
            __classPrivateFieldGet(this, _Session_actionConfirmation, "f").pending = ActionPendingState.WAIT;
    }
    /**
    ...
    getActionCofirmation() {
        if (__classPrivateFieldGet(this, _Session_actionConfirmation, "f").pending === ActionPendingState.NONE)
            throw new Error('No action currently pending confirmation!');
        if (__classPrivateFieldGet(this, _Session_actionConfirmation, "f").pending === ActionPendingState.WAIT)
            return null;
        if (__classPrivateFieldGet(this, _Session_actionConfirmation, "f").pending === ActionPendingState.DONE) {
            __classPrivateFieldGet(this, _Session_actionConfirmation, "f").pending = ActionPendingState.NONE;
            return __classPrivateFieldGet(this, _Session_actionConfirmation, "f").confirm;
        }
    }
    /**
    ...
    setActionConfirmation(confirmValue) {
        ...
        __classPrivateFieldGet(this, _Session_actionConfirmation, "f").confirm = confirmValue;
        __classPrivateFieldGet(this, _Session_actionConfirmation, "f").pending = ActionPendingState.DONE;
        if (confirmValue)
            sendMessage('§qConfirmed action!', '§2BT§r', player);
        else
            sendMessage('§mCancelled action!', '§2BT§r', player);
    }
    //## Action State
    /**
    ...
    updatePointerBlockLocation() {
        ...
        switch (this.pointerMode) {
            ...
            case PointerMode.FACE:
                {
                    ...
                    //);
                    this.pointerBlockLocation = sumVectors(player.getBlockFromViewDirection().location, FACE_DIRECTIONS[getDirectionFace(player.getViewDirection())]);
            ...
            case PointerMode.FREE:
                {
                    this.pointerBlockLocation = floorVector(sumVectors(player.getHeadLocation(), setVectorLength(player.getViewDirection(), this.config.pointer.range)));
                }
                break;
    ...
    processPasteAction() {
        if (this.pasteState.clipboardIndex == null)
            return;
        const clipboard = this.getClipboard();
        clipboard.getAllParticles((particle) => {
            spawnParticleLine('trebesin:line_flex2', this.pasteState.dimension, VectorMath.sum(this.pasteState.originLocation, particle.location), particle.direction, particle.length, 0.051, particle.color);
        }, this.pasteState.clipboardIndex);
        const confirm = this.getActionCofirmation();
        if (confirm != null) {
            if (confirm)
                this.pasteSelection(this.pasteState.originLocation, this.pasteState.dimension, this.pasteState.clipboardIndex);
     ...
     */
    getClipboard() {
        return __classPrivateFieldGet(this, _Session_clipboard, "f");
    }
    /**
...
_Session_player = new WeakMap(), _Session_clipboard = new WeakMap(), _Session_actionConfirmation = new WeakMap();
//# Clipboard:
class ClipboardInstance {
    ...
    getBlockStateIndex(blockState) {
        ...
        if (indexInArray >= 0)
            ...
            return (this.blockStateData.push(blockState) - 1);
    }
    /**
     *
    ...
    updateParticlePreview(clipboardIndex) {
        ...
        const particleCenter = VectorMath.divide(VectorMath.sum(VectorMath.getMaximalVector(particleBounds), VectorMath.getMinimalVector(particleBounds)), 2);
