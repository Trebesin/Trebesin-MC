        ...
        switch (parameter.command) {
            ...
            case "show":
                ...
                }
                {
                    if (lastParticleCall[sender.id]) {
                        ...
                        sendMessage("there is nothing to show", "blockHistory", sender);
                }
                break;
            case 'b':
            case 'block':
            ...
            case 'player':
                ...
                }
                {
                    ...
                    if (!printBlockHistory(response, { type: "player" }, sender))
                        break;
                    lastCall[sender.id] = response.result;
                    sendMessage('You can use !bh warp [index] to warp to a block', "blockHistory", sender);
                    if (parameter.particles ?? false) {
            ...
            case 'redo':
                ...
                }
                {
                    ...
                    if (!printBlockHistory(response, { type: "reverse" }, sender))
                        break;
                    lastCall[sender.id] = response.result;
                    sendMessage('You can use !bh warp [index] to warp to a block', "blockHistory", sender);
                    if (parameter.particles ?? false) {
...
function printBlockHistory(request, options, sender) {
    ...
    if (options.type === "reverse")
        sendLongMessage(`Block History reverses of ${playerName}`, message.trim(), sender);
    if (options.type === "player")
        sendLongMessage(`Block History of ${playerName}`, message.trim(), sender);
    if (options.type === "block")
        sendLongMessage(`Block History of block ${Math.floor(options.pos.x)}, ${Math.floor(options.pos.y)}, ${Math.floor(options.pos.z)}`, message.trim(), sender);
    return true;
}
