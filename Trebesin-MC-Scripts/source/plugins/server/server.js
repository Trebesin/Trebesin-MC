...
export async function main() {
    ...
    system.runInterval(() => {
        for (const player of world.getPlayers()) {
            if (playerData.instaKill[player.id]) {
                ...
                player.dimension.spawnParticle('trebesin:selection_dot_fast', location, molang);
            }
            ;
    ...
    world.beforeEvents.itemUseOn.subscribe((eventData) => {
        if (eventData.source.hasTag('certified_builder'))
            return;
        ...
        if (itemId === 'minecraft:lava_bucket' ||
            ...
            if (isAdmin(eventData.source))
                sendMessage(`§a You hovewer do have the permissions to place lava buckets. Use !allowbuild @s to grant them§r\n`, 'SERVER', eventData.source);
