...
export async function main() {
    ...
    Mc.system.runInterval(() => {
        for (let index = 0; index < fallingBlocksTracked.length; index++) {
            ...
            if (fallingBlockData == null)
                continue;
            const fallingBlockEntity = getEntityById(fallingBlockData.id, { type: 'minecraft:falling_block' }, [fallingBlockData.dimensionId]);
            if (fallingBlockEntity == null) {
                ...
                Debug.sendLogMessage(`§aBlock Ends Falling§r [${location.current.x},${location.current.y},${location.current.z}] @ ${tick.current} - ${getEntityById(fallingBlockData.playerId, {}, [fallingBlockData.dimensionId])?.nameTag}`);
            ...
            if (player.hasTag('inspector')) {
                try {
                    ...
                    if (getEquipedItem(player) != null)
                        ...
                        BlockHistoryCommandsWorker.inspector(blockLocation, player);
                }
                catch (error) {
    ...
    Mc.world.beforeEvents.itemUseOn.subscribe((eventData) => {
        ...
        oldBlocksRegister[vec3ToString(eventData.block.faceLocation)] = Blocks.copyBlockState(eventData.block.dimension.getBlock(eventData.faceLocation), true);
