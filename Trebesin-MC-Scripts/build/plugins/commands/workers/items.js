...
export function main() {
    ...
    Mc.world.beforeEvents.itemUse.subscribe((eventData) => {
        ...
        if (eventData.itemStack.typeId === 'trebesin:cmd_universal') {
            ...
            Mc.system.runTimeout(() => Commands.runCommand(command, parameters, eventData.source), 1);
        }
        //## Hardcoded items:
        const itemCommand = itemCommands[eventData.itemStack.typeId];
        if (itemCommand) {
            Mc.system.runTimeout(() => Commands.runCommand(itemCommand.input, itemCommand.parameter, eventData.source), 1);
        }
    });
}
