...
export async function main() {
    ...
    Commands.registerCommand('fillReplaceSelection', {
        ...
        async run(sender, parameters) {
            ...
            const session = Sessions.getSession(sender);
            session.fillReplaceSelection(parameters.fillBlock, replacePermutations, parameters.replaceMode === 'exclude');
        }
    });
    ...
    Commands.registerCommand('setblock', {
        ...
        run(sender, parameters) {
            setBlockPermutation(sender.dimension.getBlock(parameters.location), parameters.fillBlock.permutation, { actorId: sender.id, updateType: 'blockyTools: player' });
        }
    });
    ...
    Commands.registerCommand('movePasteOrigin', {
        ...
        run(sender, parameters) {
            ...
            session.pasteState.originLocation = VectorMath.sum(session.pasteState.originLocation, {
                x: parameters.x,
                y: parameters.y,
                z: parameters.z
            });
        ...
        run(sender, parameters) {
            try {
                ...
                for (let index = 0; index < blocks.length; index++) {
                    ...
                    setBlockType(sender.dimension.getBlock(block), Mc.MinecraftBlockTypes.get(parameters.blockId), { actorId: sender.id });
                }
