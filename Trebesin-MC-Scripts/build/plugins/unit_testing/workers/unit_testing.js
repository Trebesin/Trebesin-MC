...
const unitTestingList = { child: [
        ...
        { name: 'commands', child: [
                ...
                { name: '!tphere', run: (sender) => { Commands.runCommand('tphere', '@a', sender); } },
            ...
            try {
                switch (parameters.command) {
                    case 'next':
                        ...
                        if (currentActiveUnitTestingPerPlayer[sender.id].position[currentActiveUnitTestingPerPlayer[sender.id].position.length - 1] + 1 >= getObjectFromIndex(unitTestingList, newArrayWithoutTheLastElement).child.length) {
